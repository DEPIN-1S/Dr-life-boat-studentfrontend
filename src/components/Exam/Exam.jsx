...
const Exam = () => {
  ...
  // Mark exams as completed and merge relevant data
  const allExams = availableExams.map((exam) => {
    const submittedExam = submittedExams.find((sub) => sub.se_exam_id === exam.ex_id);
    return {
      ...exam,
      is_completed: !!submittedExam,
      se_exam_id: submittedExam ? submittedExam.se_exam_id : null,
      se_duration: submittedExam ? submittedExam.se_duration : null, // Add se_duration for completed exams
    };
  });
            ...
            <div key={exam.ex_id} className="col-lg-4 col-md-6 col-sm-12 mb-4 d-flex justify-content-center">
              ...
              >
                ...
                <div className="card-body d-flex flex-column justify-content-center text-center">
                  ...
                  {exam.is_completed ? (
                    <div>
                      <span className="badge bg-success-subtle text-success px-3 py-2">Submitted</span>
                      {exam.se_duration && (
                        <p className="mt-2 mb-0" style={{ fontSize: '0.85rem' }}>
                          Time Taken: {exam.se_duration}
                        </p>
                      )}
                    </div>
                  ) : (
